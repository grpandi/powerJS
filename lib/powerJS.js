...
class Pjs {  
  ...
  async readFile(files){
    ...
    this.zip = await zip.loadAsync(f)
    ...
    this.zip.folder('ppt/slideLayouts').forEach(async (path,file)=>{
     ...
     this.zip.folder('ppt/slideMasters/_rels').forEach(async (path,file)=>{
      ...
      this.getSchemeClr()
      this.slides.getBG(1)
      // console.log(this.slides.getSlideCount())
  }
  ...
  getSchemeClr(){
    ...
    let Clrs = this.getNested(this.theme, 'a:theme','a:themeElements','a:clrScheme')
    this.SchemeColors = {}
    for (let v in Clrs){
      let clr = this.getNested(Clrs, v,'a:sysClr','@_lastClr')
      if(typeof(clr) !='undefined'){this.SchemeColors[v]=clr}
      clr = this.getNested(Clrs, v,'a:srgbClr','@_val')
      if(typeof(clr) !='undefined'){this.SchemeColors[v]=clr}
    }
  }
